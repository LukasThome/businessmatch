...
function remove() {
  ...
  list.then((l) => {
    l.map(({ id }) => {
      const propostaId = document.getElementById(`propostaId__${id}`);
      if (propostaId) {
        const listener = () => {
          eel.removeProposta(id)();
          location.reload();
          propostaId.removeEventListener("click", listener);
        };
        propostaId.addEventListener("click", listener);
      }
    });
  });
}
...
function aceitar(id) {
  ...
  list.then((propostas) => {
    if (propostas) {
      ...
      if (proposta) {
        proposta.status = 1; // Define o status como 1 (ou qualquer outro valor que represente "Aceitar")
        eel.updateProposta(proposta)(); // Chama uma função no servidor Python para atualizar a proposta
        location.reload(); // Recarrega a página após a ação ser concluída
      }
    }
  });
}
  function recusar(id) {
    ...
    list.then((propostas) => {
      if (propostas) {
        const proposta = propostas.find((p) => p.id === id);
        if (proposta) {
          proposta.status = 0; // Define o status como 0 (ou qualquer outro valor que represente "Recusar")
          eel.updateProposta(proposta)(); // Chama uma função no servidor Python para atualizar a proposta
          location.reload(); // Recarrega a página após a ação ser concluída
        }
      }
